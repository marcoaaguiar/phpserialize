...
class PhpSerializeTestCase(unittest.TestCase):
    ...
    def test_dumps_str(self):
        self.assertEqual(phpserialize.dumps("Hello world"), b's:11:"Hello world";')

    def test_dumps_unicode(self):
        self.assertEqual(
            phpserialize.dumps("Björk Guðmundsdóttir"),
            b's:23:"Bj\xc3\xb6rk Gu\xc3\xb0mundsd\xc3\xb3ttir";',
        )

    def test_dumps_binary(self):
        self.assertEqual(phpserialize.dumps(b"\001\002\003"), b's:3:"\x01\x02\x03";')

    def test_dumps_list(self):
        self.assertEqual(
            phpserialize.dumps([7, 8, 9]), b"a:3:{i:0;i:7;i:1;i:8;i:2;i:9;}"
        )
    ...
    def test_loads_dict(self):
        self.assertEqual(
            phpserialize.loads(
                b'a:3:{s:1:"a";i:1;s:1:"c";i:3;s:1:"b";i:2;}', decode_strings=True
            ),
            {"a": 1, "b": 2, "c": 3},
        )
    ...
    def test_dumps_and_loads_dict(self):
        self.assertEqual(
            phpserialize.loads(
                phpserialize.dumps({"a": 1, "b": 2, "c": 3}), decode_strings=True
            ),
            {"a": 1, "b": 2, "c": 3},
        )
    ...
    def test_reference(self):
        ...
        self.assertEqual(result, {0: b"foo", 1: b"foo"})

    def test_reference_nested(self):
        data = b'a:2:{s:2:"na";a:2:{s:2:"te";s:2:"st";s:2:"xa";s:2:"xi";}s:9:"refstring";R:4;}'
        result = phpserialize.loads(data)

        self.assertEqual(
            result, {b"na": {b"te": b"st", b"xa": b"xi"}, b"refstring": b"xi"}
        )
